...
wykres_wlkp <- ggplot(data = google_wlkp, aes(x = date, y = parki)) + 
  ...
  geom_vline(xintercept = as.Date("2020-03-20"), linetype = "dotted", color = "red") +
  geom_vline(xintercept = c(as.Date("2020-07-01"), as.Date("2020-08-31")), color = "black") +
  theme_bw() +
  labs(x = "Data", y = "Indeks popularności parków", 
...
```{r}
google_west_pom <- google %>% 
  filter(sub_region_1 == "West Pomeranian Voivodeship", !is.na(sub_region_2)) %>%
  select(powiat = sub_region_2, date, 
         handel_i_rekreacja = retail_and_recreation_percent_change_from_baseline,
         sklepy_i_apteki = grocery_and_pharmacy_percent_change_from_baseline, 
         parki = parks_percent_change_from_baseline,
...
wykres_wlkp %+% 
  google_west_pom + 
  labs(title = "Indeks popularności parków w Zachodniopomorskim według Google Mobility Index") 
```
...
google_wlkp <- google_wlkp %>% 
  mutate(miast_pow = powiat %in% c("Kalisz County", "Konin County", "Poznań County", "Leszno County"))
